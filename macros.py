...
df[[
    "Lean_Score_MCR_norm",
    "Energy_Score_MCR_norm",
    "Satiety_Score_MCR_norm"
...
# --- Function to Get Top 5 per Mood and Veg/Non-Veg ---
def get_top_5(df, mood, score_col, veg_status):
    if mood in df["Cluster_Label"].unique():
        mood_df = df[(df["Cluster_Label"] == mood) & (df["Veg/Non-Veg"] == veg_status)]
...
for mood, score_col in moods.items():
    for veg_type in ["Veg", "Non-Veg"]:
        ...
        print(f"\n{emoji} Top {veg_type} Dishes to Feel {mood}:\n", top_dishes[["Item Name", score_col, "Protein (g)", "Fat (g)", "Carbs (g)"]])
